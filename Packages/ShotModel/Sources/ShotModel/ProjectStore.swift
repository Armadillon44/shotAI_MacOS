...
public actor ProjectStore {
    ...
    /// Does this title look like one WE generated, rather than one a human chose?
    ///
    /// Kept beside `defaultTitle()` so the pattern and the generator cannot drift.
    /// Used by the SOP prompt: asking Claude to judge whether a name "already
    /// reads as a descriptive procedure title" is a judgement it gets wrong in
    /// both directions, and the app already knows the answer for the common case.
    public static func isAutoGeneratedTitle(_ title: String) -> Bool {
        let t = title.trimmingCharacters(in: .whitespacesAndNewlines)
        if t.isEmpty { return true }
        return t.range(of: #"^Project \d{4}/\d{2}/\d{2} \d{2}:\d{2}:\d{2}$"#,
