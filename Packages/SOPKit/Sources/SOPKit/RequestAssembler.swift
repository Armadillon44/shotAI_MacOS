...
func assembleRequest(
    ...
    header += "stepNumber to the number in that block's own heading and its kind to "
    header += "\"screenshot\" or \"text\". The numbers count every block, so screenshots are "
    ...
    for (idx, step) in source.enumerated() {
        ...
        if step.kind == .text {
            ...
            // Sent from the PRE-AI original, as captions are, so a regeneration never
            // feeds Claude its own earlier rewrite as if the author wrote it — each
            // run would drift further from what the author said. The exception is
            // the same one captions have: the author edited the block after a
            // generation, which is a deliberate correction to rewrite from.
            let source = step.captionEditedByUser == true ? step : (originalById[step.id] ?? step)
            var parts = ["--- \(AssembledRequest.authorBlockLabel(step.callout)) \(n) (author-written) ---"]
            if let h = source.heading, !h.isEmpty { parts.append("Heading: \(h)") }
            if let b = source.body, !b.isEmpty { parts.append("Body: \(b)") }
