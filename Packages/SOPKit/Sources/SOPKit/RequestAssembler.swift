...
func assembleRequest(
    ...
    header += "SCREENSHOT step, setting its stepNumber to the number in that step's own "
    header += "\"--- Screenshot step N ---\" heading. Those numbers count the author's text "
    header += "blocks too, so they are often NOT 1, 2, 3 and may skip values — copy each "
    header += "number exactly as shown and never renumber them. An entry whose stepNumber "
    header += "does not match a screenshot heading is discarded. Keep the "
