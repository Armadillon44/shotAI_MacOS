...
func assembleRequest(dir: String, manifest: ProjectManifest, settings: SopSettings) throws -> AssembledRequest {
    ...
    // Do not make Claude judge whether the name is descriptive — it gets that
    // wrong in both directions. Once it returned the literal word "placeholder"
    // (that word used to sit beside the value); once it kept "Project 2026/08/19
    // 12:41:16" verbatim. The app knows which case this is, so just say so.
    if ProjectStore.isAutoGeneratedTitle(manifest.title) {
        header += "That name was generated by the app from a timestamp. It is NOT a title, "
        header += "carries no meaning, and MUST be replaced. Set `title` to a clear, specific "
        header += "name for the PROCEDURE ITSELF, derived from what the steps accomplish — for "
        header += "example \"Configuring VPN access in the admin console\". Never return a "
        header += "generic or filler title such as \"placeholder\", \"untitled\", \"SOP\" or "
        header += "\"Project\", and never return the current name; if the steps are sparse, "
        header += "still name the specific task they show.\n"
    } else {
        header += "That name was written by a person, so treat it as intentional. Set `title` "
        header += "to it unchanged unless it is clearly wrong or vague for what the steps "
        header += "actually accomplish, in which case improve it while keeping their meaning.\n"
    }
