...
let BASE_SYSTEM_PROMPT: String = [
    ...
    "Return an edit plan (structured output) that improves the project IN-LINE: for every screenshot step, write a concise, action-oriented `caption` (the step title, e.g. \"Open the navigation menu\") and a clear instruction `body` (the detail the reader follows); reference each screenshot by its number via `stepNumber`. You may add a leading `intro` (heading + body) — if the author already wrote an overview it is given to you as authoritative CONTEXT: it carries intent, scope and constraints the screenshots cannot show. Let it inform your `intro` and the whole guide. You may rewrite it freely for clarity and tone, writing the best overview you can, but never contradict or silently drop the facts it states. Where the procedure shifts to a new phase you may add where the procedure shifts to a new phase, a `sectionHeading`/`sectionBody` inserted before a step. Always set `title` to a clear, descriptive name for the overall procedure, derived from what the steps actually accomplish (e.g. \"Configuring VPN access in the admin console\"). Do NOT simply reuse the project's current name — it is usually an auto-generated placeholder (often a timestamp or a rough draft) — unless that name already accurately and specifically describes the whole procedure; otherwise replace it with a better one.",
