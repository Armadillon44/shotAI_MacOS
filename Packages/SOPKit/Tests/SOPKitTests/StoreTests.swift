...
final class IntroAuthorshipTests: XCTestCase {
    ...
    /// Applying a generation clears the flag, so the next run does not treat the
    /// model's new overview as the author's.
    func testApplyingAGenerationClearsTheIntroFlag() async throws {
        let (store, path, _) = try await makeProject(shots: 1)
        _ = try await store.setIntro(at: path, heading: "Mine", body: "Author text.")
        let flagged = try await store.openProject(at: path).manifest.introEditedByUser
        ...
        _ = try await SOPKit.applySopEdits(store: store, projectPath: path, plan: plan,
                                           model: .sonnet5, tone: .professional)
        let after = try await store.openProject(at: path).manifest
        XCTAssertEqual(after.intro?.body, "AI text.")
        XCTAssertNil(after.introEditedByUser, "the model overwrote it — flag must clear")
    }
