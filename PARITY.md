...
**Audit date:** 2026-07-28 · **Target:** shipped Windows **v1.1.4** · **Reference:** `shotAI-original/` (v1.1.4 / `d9823ec`) · **macOS build:** `1.3.0` (build 11)
...
| Report / SOP viewer | ✅ matched | Full editing loop — inline caption/instruction + editable title, insert/import/delete/reorder/merge, text steps, callouts, **section dividers** (both apps), per-step zoom + drag-to-pan, overview intro, **per-project document scale**. |
| Document scale | ✅ matched | Per-project 65–125% in 5% detents, persisted as `displayScale` in `project.json` (omitted at 100%, so existing projects are untouched and round-trip byte-identically). Shipped first on Windows (`Armadillon44/shotAI#72`); macOS ported the snap algorithm, the detent table and every derived width verbatim (`DocScale.swift` is the shared contract). Slider + typed entry + ↑/↓ + stepper, honored by the report and all four exports. macOS-only detail: PDF is shrink-only, since the page width is fixed. |
...
| Export dimensions | ✅ matched | `.doc` 880 outer / 816 column / 738 image — Windows v1.1.3 explicitly ported these to match macOS. **Exports are byte-unchanged by the document-scale work at 100%.** One accepted difference: `ReportPresentation.baseWidth` (which the Windows source cites) moved 820 → 816 so the macOS report column IS the export column. It is an on-screen constant only — no export path reads it — and it is inert even there, since the measured card width binds first. macOS's report is SwiftUI re-deriving the geometry, where Windows's report is the same HTML as its export and matches for free. |
...
| Authentication (Entra work-account sign-in) | ✅ matched | Both apps sign a user in with their Microsoft work account and call Anthropic with a federated token, so a signed-in user needs no Anthropic API key. macOS shipped first (`docs/SSO-WIF.md`); Windows landed it in `Armadillon44/shotAI`#63, **merged to `main` but not yet in a Windows release** (latest shipped is v1.1.6). Access is an Entra app-role assignment (`shotAI.User`) enforced by a condition on the Anthropic federation rule, so removing the assignment revokes Claude access at the next token exchange, with no app change and no key rotation. Both keep bring-your-own-key as the fallback; on Windows it stays reachable behind a **Use my own Anthropic API key instead** disclosure, and being signed in takes precedence over a stored key (`ANTHROPIC_API_KEY` is explicitly nulled while signed in). **Windows-ahead diagnostic:** every access denial from Anthropic is deliberately the same opaque `401`, so Windows splits **Test connection** into three named legs (Microsoft sign-in / Claude access / Claude API) to say which one failed; macOS has no documented equivalent. Settings delivery differs by platform, see the next row. |
| Managed configuration (MDM) | ✅ matched | Same design on both: the federation settings are **baked into the build** (macOS a gitignored `Federation.plist`, Windows into the installer), with a managed override for the one case a build cannot cover, a rotated federation rule. Delivering nothing is valid on both, and both fail closed on an incomplete set rather than attempting a half-configured sign-in. **Windows** reads `REG_SZ` values from an HKLM policy key (HKLM only, never HKCU, 64-bit view queried explicitly), delivered by ADMX through Intune or Group Policy; a blank value counts as "not set" so it cannot clear a baked one. **macOS** reads managed preferences (`objectIsForced`). Two divergences worth knowing before authoring a profile: (1) the **key names are entirely different** and do not interchange, `TenantId` vs `federationEntraTenantId` and so on, so a profile written from one platform's names delivers nothing on the other; (2) the **required sets differ**, macOS requires all seven of its keys (`FederationConfig.Key.all`) while Windows has eight names of which five are required, treating `ClientAppId` and `WorkspaceId` as optional, and having a `SupportUrl` macOS has no equivalent for. Mapping table: Windows `docs/MANAGED-CONFIG.md`; see also Windows `Intune/Windows/README.md` and macOS `docs/SSO-WIF.md`. |
| SOP author context | ✅ matched | macOS already did this (`RequestAssembler`); Windows caught up in `Armadillon44/shotAI`#62, **merged to `main`, not yet released**. The author's own material is announced to Claude by kind and number, as `--- Warning callout 4 (author-written — leave this content alone) ---`, so a red warning is no longer indistinguishable from an ordinary paragraph. Per-kind guidance accompanies the **callouts and section headings** (a section heading gets phase-boundary guidance; note / caution / warning share one about informing surrounding steps); a **plain text step gets no extra guidance** by design, since its label already says enough. A caption the user edited after a generation is sent as the **user's** text rather than the original machine caption. |
| Authored overview preserved | 🔴 macOS gap | Windows flags an overview a human saved (`introEditedByUser`, set only on the human edit path) and then pins the author's **heading verbatim**, accepts the body only as a reword (clarity, grammar, voice, may add context, must not restructure, replace, or drop any stated fact or constraint), keeps the author's overview when the model returns none instead of deleting it, still rewrites freely an overview Claude wrote, and restores the flag on revert (`Armadillon44/shotAI`#64, merged to `main`). macOS has no equivalent rule, so generation can still replace an author-written overview, which is what prompted this on Windows: a user reported "Claude still re-wrote my overview; heading and all". Tracked as [#80](https://github.com/Armadillon44/shotAI_MacOS/issues/80), which carries the Windows implementation detail. Note the field is additive and optional, so `project.json` round-trips either way; until macOS lands it, an overview authored on one platform opens unprotected on the other. |
