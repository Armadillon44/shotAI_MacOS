...
| Authored overview preserved | ✅ matched | **Closed 2026-08-26 ([#80](https://github.com/Armadillon44/shotAI_MacOS/issues/80)).** macOS carries `introEditedByUser` and uses it: `SOPKit/RequestAssembler.swift:73` picks the overview by the flag and `SopApply.swift:42,46` preserves it across a generation, so an author-written overview is no longer replaced. **One divergence remains and it is Windows-side:** when the flag is NOT set, Windows still sends `manifest.intro` unconditionally, which after any generation is Claude's own text being fed back as the author's ([Armadillon44/shotAI#79](https://github.com/Armadillon44/shotAI/issues/79)). |
